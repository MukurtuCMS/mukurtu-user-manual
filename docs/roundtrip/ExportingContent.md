...
There are two ways to export content. An export list lets you gather items over time into a named, reusable, and shareable group before exporting them — see [Using Export Lists](UsingExportLists.md) for how to build and manage one. An ad hoc export skips that step and exports one or more items right away. Either way, the export itself uses an export setting that controls exactly which fields are included.
