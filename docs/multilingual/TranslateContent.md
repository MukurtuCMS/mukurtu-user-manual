---
tags:
    - multilingual
---

# Translate Content

!!! roles "User roles"
    Mukurtu manager, protocol steward, language steward, contributor, language contributor, curator, community record steward

Once the Mukurtu Multilingual module is installed and a language is added see [Enable Multilingual Module](../multilingual/EnableMultilingualModule.md), site content can be translated.

If a user can create or edit a piece of content, they can also translate it - additional roles or permissions are not required. 

Content translation functions the same way for all content types. 

1. Navigate to the content you want to translate and select the "Translate" button.

    ![Screenshot of a digital heritage item with the Translate button highlighted.](../_embeds/translatecontent1.png)

2. Select the "Add" button to the right of the language you want to translate your content into. 

    ![Screenshot of the language selection page with the Add button highlighted.](../_embeds/translatecontent2.png)

3. The fields appear with text in the original language for ease of reference. Replace this text with your preferred translation.

    ![Screenshot of the translation page for a digital heritage item with the original English text in the title field.](../_embeds/translatecontent3.png)

4. When you complete your translation, select the "Save (this translation)" button in the top right-hand corner to save your translation.

    ![Screenshot of the translation page for a digital heritage item with the updated French translation in the title field.](../_embeds/translatecontent4.png)

5. This is what a translated digital heritage item looks like. 

    ![Screenshot of the translated digital heritage item in French.](../_embeds/translatecontent5.png)