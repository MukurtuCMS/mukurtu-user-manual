...
# Create and manage User Accounts from a Site-Wide Role
...
Method 3: Add/admin/people/create to your url in your browser's address bar.
...
- Blocked members cannot log in to the site. They are typically blocked for two reasons:
    ...
    2. When a user takes inappropriate actions that warrants a temporary loss of access to the site. To access the site again, their status must be changed to "active."

5. Select a site-wide role for the user. By default, they are assigned the authenticated user role, and this is the correct setting for the majority of users. This role allows them to view public material and save content to their own personal collections. The administrator and Mukurtu manager roles are for users who manage the site and therefore have additional access to controls and settings. Assign these roles with extreme caution.

    !!! tip
	    For detailed information about user account requests, see [User Account Requests](Link)

    !!! tip
	    For more information on user roles, see [User roles](user roles article)
...
Make any desired edits and select **Save**. Refer to the [Create a new user account](#create-a-new-user-account) section for detail on each field.
